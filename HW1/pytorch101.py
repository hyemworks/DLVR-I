...
def multiples_of_ten(start, stop):
  ...
  # assert {condition} {message} : If {condition} is False, return AssertError.
  
  
  start_val = (start + 9)//10 *10

  if (start_val > stop):
      x = torch.zeros(0)
  else:
      # x = torch.arange(start_val, stop + 1, step=10, dtype=torch.float64)
      # ㄴ dtype not working
      x = torch.arange(start_val, stop + 1, step=10)
      
  x = x.to(torch.float64)
  
  # torch.arange(start=0, stop, step=1, dtype, ...)
...
def reverse_rows(x):
  ...
  # https://includestdio.tistory.com/32 : reversed()
...
def count_negative_entries(x):
  ...
  # count 함수를 사용하려고 했으나, 본 게시물에 소개되어있는 내용처럼
  # Bool 중 True의 값이 카운팅 되지 않았고, 출력했을 때 아래와 같은 결과물만 나왔음.

  # [Result]
  # Correct for x0:  tensor([[False, False, False],
  #         [False, False, False],
  #         [False, False, False]])
  # Correct for x1:  tensor([True, True, True, True])
  # Correct for x2:  tensor([[False, False, False,  ..., False, False, False],
...
def reshape_practice(x):
  ...
  # step : (24,) => reshape(2,3,4) => permute(1,0,2) => reshape(3,8)
  
  y = x.reshape(2,3,4).clone()
  #print(y)
  y = y.permute(1,0,2)
  #print(y)
  y = y.reshape(3,8)
...
def zero_row_min(x):
  ...
  mask = torch.argmin(x, dim=1) # [0, 2]
  print(mask)
  
  y = x.clone()
  y[range(x.shape[0]), mask] = 0 # [row_info, column_info]
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  # x와 y의 배치데이터에서 "각 단일 데이터"를 하나씩 꺼내서 연산 후, 결과물을 
  # 다시 배치 상태로 돌려 놓았다고 할 수 있는 듯!
  ...
  if use_loop == True:
      ...
      for b in range(B):
          z = torch.stack([torch.mm(x[b], y[b])], dim=0)
...
def normalize_columns(x):
  ...
  c_mean = torch.sum(x, dim=0)/M
  #print("c_mean : ", c_mean)
  
  c_stdd = torch.sqrt((1/(M-1)) * torch.sum((x-c_mean)**2, dim=0))
  #print("c_stdd : ", c_stdd)
...
def mm_on_gpu(x, w):
  ...
  # (i) Place each input on GPU
  x_gpu = x.clone().detach().cuda()
  w_gpu = w.clone().detach().cuda()

  # (ii) Perform the matrix multiplication operation & (iii)
