...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    loss = None # Initialize the variable 'loss'
    ############################################################################
    ...
    ## 01. Compute data loss
    #scores = scores - torch.max(scores, dim=1, keepdims=True) # https://jimmy-ai.tistory.com/70
                                             # Numerical stability 왜 오류날까... 
    max_score = torch.max(scores, axis=1)[0].reshape(-1, 1)
    #print("max_score : " , max_score) #debug    
    scores = scores - max_score
    #print("edited_score : ", scores)  #debug      
                                                        
    exp_scores = torch.exp(scores)
    #print("exp_scores : ", exp_scores)  #debug # scores가 굉장히 작은 수여서, exp연산을 하면 1로 수렴해버리는 듯?
    prob_exp = exp_scores / torch.sum(exp_scores, axis=1, keepdims=True) # Normalize exp_scores for each score
    #print("prob_exp : ", prob_exp)  #debug
    
    #print("prob_exp[range(N),y] : ", prob_exp[range(N),y])  #debug
    
    correct_logprobs = -torch.log(prob_exp[range(N),y]) 
    #print("correct_logprobs : ", correct_logprobs)  #debug
    
    data_loss = torch.sum(correct_logprobs) / N
    #print("data_loss : ", data_loss)  #debug
    
    ## 02. Compute L2 regularization    
    reg_loss = reg*torch.sum(W1**2) + reg*torch.sum(W2**2) # Broadcasting
    #print("reg_loss : ", reg_loss)  #debug
    
    ## 03. Compute loss
    loss = data_loss + reg_loss # == scalar
    
    # In this part, you have to update the variable 'loss'
    ...
    grads = {}  # Initialize the variable 'grads'
    ###########################################################################
    ...
    # dy = torch.ones(scores.shape, dtype=torch.float32, device="cuda") # Initialize gradient
    # RuntimeError : module 'torch' has no attribute 'double64'
    # torch.mm --- This operator supports TensorFloat32. #https://pytorch.org/docs/stable/generated/torch.mm.html
    dy = prob_exp.clone()
    #print("dy : ", dy)
    dy[range(N), y] -= 1 # 직접 미분해보면 해당 식을 도출할 수 있음!
    #print("dy : ", dy)
    ...
    # FC layer 01
    #dX = torch.mm(dhidden, W1.T)
    dW1 = X.T.mm(dhidden)
    db1 = torch.sum(dhidden, axis=0)
    
    # In this part, you have to update the variable 'grads'
